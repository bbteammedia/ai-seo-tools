...
class RunHistoryPage
{
    ...
    public static function render_history()
    {
        ...
        foreach (glob($rdir . '/*', GLOB_ONLYDIR) as $runPath) {
            ...
            $images = is_array($sum) ? intval($sum['images'] ?? 0) : count(glob($runPath . '/images/*.json'));
            $errors = is_array($sum) ? intval($sum['errors'] ?? 0) : count(glob($runPath . '/errors/*.json'));
        ...
        foreach ($runs as $r) {
            ...
            echo '<td>' . esc_html($r['images']) . '</td>';
            echo '<td>' . esc_html($r['errors']) . '</td>';
